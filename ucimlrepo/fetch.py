...
# API endpoints
API_BASE_URL = 'http://archive.ics.uci.edu/api/dataset'
API_LIST_URL = 'http://archive.ics.uci.edu/api/datasets/list'

# base location of data csv files
DATASET_FILE_BASE_URL = 'http://archive.ics.uci.edu/static/public'
...
def fetch_ucirepo(
    ...
    data_url = metadata['data_url'].replace("https", "http")
...
def list_available_datasets(filter: Optional[str] = None, search: Optional[str] = None):
    ...
    data = None
    try:
        response  = urllib.request.urlopen(api_list_url, context=ctx)
